...
`SearchKnowledgeScope` 最多返回 8 个候选的标题、短 snippet、命中原因和 `sourceRef`，不返回整篇正文。`ReadKnowledgePassages` 每次最多读取 3 个引用。`sourceRef` 保存于本轮 Run 的临时 registry，绑定 `runId + scopeId + path + anchor + contentHash`；模型不能伪造绝对路径、`../` 或另一个 Scope 的引用。
